...
M3 begins with a deliberately narrow deterministic PNG adapter. The current adapter validates
PNG header metadata, the complete bounded chunk stream, and the zlib/DEFLATE data carried by
`IDAT`. It verifies the exact decompressed scanline byte count, including Adam7 sizing, before
emitting exact-source Artifact IR. The bytes remain PNG-filtered: SightLint still does not claim
pixel colors, ink bounds, text, components, roles, or other semantics at this stage.
...
# Validate a PNG through bounded chunk parsing and inflation, then emit canonical Artifact IR.
...
them through binary stdin. Their zlib fixtures use an independent test-only stored-DEFLATE
encoder rather than the production inflater, so decompression behavior is not self-validated.
