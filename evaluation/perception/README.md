...
`benchmark-image-segmentation` command produces bounded pixel candidates, and the local reference
worker exposes only the qualified-policy regions as `visionMeasured`. Native and pixel outputs are
retained separately during evaluation; neither rewrites the other or the annotations.
