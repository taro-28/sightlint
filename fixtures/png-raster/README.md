...
`corpus.json` contains 43 small, procedural native-input cases. Hex strings are committed PNG
...
- 5 cases: transparent borders, translucent-only/disconnected samples, an internal hole, edge
  occupancy, and two hidden-RGB variants with no visible samples
...
Every case reaches the native adapter API and both public image commands. Supported pixels and
source-alpha geometry are compared to the independent oracle; the CLI checks its encoded-sample checksum,
...
No display color correction, alpha compositing, inferred background, roles, or peer groups are
supplied by this corpus. Alpha geometry uses encoded source alpha only and does not establish a
padding defect. Unsupported ancillary semantics are not claimed to be fully validated.
Animation-marker unavailability does not certify a complete valid APNG stream.
